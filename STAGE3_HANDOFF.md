...
阶段3 四项任务全部落地并跑通：FinMem 分层记忆、Pydantic 风格严格 schema + Function Calling、四角色智能体、受控 A/B 闸门。**五道进门验证全绿**。诚实结论：在真实行情前向样本上，四角色+FinMem 并未显著优于朴素规则（p=0.74），因此守门逻辑正确判定「回退规则」——这正是验证层「防骗自己」的价值，而非伪造 edge。
...
|------|--------|--------|-----------|-----------|
| 规则（动量逐根） | 623 | 0.603 | 40% | purge=35 embargo=25 |
| 四角色+FinMem | 429 | 0.490 | 40% | purge=25 embargo=15 |

- ΔDSR = −0.198，p = 0.7444，**不显著**，胜方=规则。
- 四角色风控/记忆过滤掉 195 笔（多为 RANGE 低质量），但本样本上未带来更优 edge。
