...
阶段4 四项任务全部落地并跑通：TSFM 预测骨架（含 torch 降级 numpy）、CVaR-约束夏普目标、StockSim LLM 驱动市场。**六道进门验证全绿**。诚实结论：受控 A/B 中 TSFM 方向策略 PSR=0.891 **低于** 动量基线 0.968，Welch + 多重检验校正后 p=0.8514 **不显著**，守门逻辑正确判定「未证明更优、不伪造 edge」——torch 虽在沙箱内可用，但无外网预训练权重，落地的仍是小蒸馏 MLP + numpy 双后端，纪律（可选 + 降级）如实达成。

> ⚠️ 数值诚信：本表数值由 `python3 -m cryptoquant_auto.run_validation_stage4` 于提交 `726b7d4` 实跑生成。早期交接文档曾误记为 动量基线 0.603 / TSFM 0.936（PS 误标为 DSR、且为旧代码态）。A/B 打印值实为 `deflated_sharpe(n_trials=1)` 即 **PSR**，且引擎以 `periods_per_year=1`（逐bar、非年化）调用，与 `sim/metrics.py` 默认 8760 不同。数值随代码/数据演进，**以实跑为准**，勿手填。
...
|------|--------|--------|-----------|-----------|
| 动量基线（sign(momentum)） | 623 | 0.968 | 40% | purge=35 embargo=25 |
| TSFM 方向（逐根一步外推） | 624 | 0.891 | 40% | purge=35 embargo=25 |
...
- ΔPSR = −0.077，p = 0.8514，**不显著**，胜方=规则（TSFM 未显著更优）。
- TSFM 方向由 `rets_full[:idx+1]`（截止当前已收盘 bar）拟合、预测 idx+1（即 forward），属真一步外推，无泄露。
- **裁决：🔒 未证明更优（不伪造 edge）**。蓝图规则——受控 A/B 不显著则不切换，沿用动量/规则基线。

> 说明：TSFM 的 0.891 是单次前向 WF 的经济 edge 实测值（非闸门机制），在 Welch t 检验 + 多重检验校正（n_trials=信号数）下未达显著。验证层价值在「防骗自己」：原型无 edge 圣杯，torch 重算力已就位但仍须受控闸门约束。
...
上述记录为阶段4 规划态。以下是实际落地后的修正：

### TSFM（任务20）
- **计划→实际**：Time-MoE/Moirai → Chronos-Bolt-tiny(主,9M) + TimesFM 2.5(精度档,200M)
- `make_tsfm` 降级链：chronos → timesfm → torch → numpy
- torch_compile=False（冷启动 3-6s 而非 12-35s）
- 预训练权重已离线缓存至 data/tsfm_cache/
- 分位区间：[0.1..0.9] 原生（80% 带），SPCI 校准

### CVaR（任务21）
- DRL 未落地，保持 scipy 解析解（SLSQP），已标注待升级
- 额外 6 项：Cornish-Fisher、Ledoit-Wolf 收缩协方差、Vol-regime 自适应 budget、DE 全局求解器、Herfindahl 约束、换手率惩罚

### StockSim（任务22）
- LLM 驱动对手盘 → 9-agent 纯 numpy 委员会
- 5 种角色：趋势、均值回归、基本面、恐慌、流动性寻求者
- LOB 层：Kyle's λ、OFI、Hawkes 自激励、DFA Hurst
- 8 项程式化事实检验（7/8 通过）
- HFTMarketMakerAgent 已实现（非默认）
- 真实 LLM 经圆桌评估不采用（延迟/成本/ROI）

### A/B 验证修正
- PSR 值以实跑为准，勿引用旧版数值
- 引擎 periods_per_year 沿用默认，stage 0.5 验证层独立运行
